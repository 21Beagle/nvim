...
do
	...
	local function open_float(buf, o)
		...
		vim.wo[win].winhighlight =
			"Normal:NormalFloat,FloatBorder:FloatBorder,Title:FloatTitle,CursorLine:Visual,EndOfBuffer:NonText"
	...
	local function float_diag_ui()
		...
		-- More padding, not fullscreen, and capped size
		local pad = 3
		local gap = 2
		...
		-- Bottom message panel height (bigger than before)
		...
		-- Horizontal split with spacing between list and code
		...
		local function render_list()
			...
			-- Then highlight ranges
			for i, it in ipairs(state.items) do
				local _icon, sev_hl = severity_icon(it.severity)
				local code = it.code and tostring(it.code) or ""

				-- icon is 1 char + following space => cols [0,2)
				pcall(vim.api.nvim_buf_add_highlight, state.list_buf, ns, sev_hl, i - 1, 0, 2)

				-- code column starts at: "I " (2) + file_w + 1 + pos_w + 1
				local code_start = 2 + file_w + 1 + pos_w + 1
